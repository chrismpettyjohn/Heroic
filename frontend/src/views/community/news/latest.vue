      ...
      <header class="news-category__header">
        ...
        <nav class="news-category__navigation">
          <ul class="news-category__list">
            ...
            <li class="news-category__item">
              <router-link :to="{ name: 'Community.News.Category', params: { id: category.id }}" v-for="category in categories" :key="category.id" class="news-category__link">{{ category.title }}</router-link>
            </li>
          </ul>
        </nav>
      </header>
