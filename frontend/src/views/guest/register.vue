<template>
    <div>
      <loading v-if="state.loading" class="login-area">We are creating your account.</loading>
      <div v-if="!state.loading" class="login-area">
        <h1>User Registration</h1>
        <p>Nice to see a new face around these parts!</p>
        <form @submit.prevent="register()">
          <div class="login_group">
            <label>Username</label>
            <input v-model="user.username" :class="{'login_error': state.error==='username'}" class="login_field">
          </div>
          <div class="login_group">
            <label>Email</label>
            <input v-model="user.mail" :class="{'login_error': state.error==='email'}" class="login_field">
          </div>
          <div class="login_group">
          ...
          <div class="login_group">
            <router-link :to="{ name: 'Guest.Login' }" class="login_link">Or Login...</router-link>
          </div>
        </form>
      </div>
    </div>  
