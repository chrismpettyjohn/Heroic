<template>
	<div>
		...
		<div v-if="state.ready">
			...
			<div class="row">
				<!-- Badges -->
				<div v-if="user.badges.length > 0" class="col-6">
					<div class="profile__card__wrapper--badges" style="width:100%;">
						<section class="profile__card__aligner">
							<div class="profile__card">
								...
								<div class="item-list--grid">
									<ul>
										<li v-for="badge in user.badges.slice(0, 5)" :key="badge.id"  v-if="badge.meta" class="item item--small item--badge">
								...
								<div class="profile__card__footer">
									<p @click="toggleModal('badges')" class="profile-modal__link">See all</p>
								</div>
							</div>
						</section>
					</div>
				...
				<!-- Friends -->
				<div v-if="user.friends.length > 0" class="col-6">
					<div class="profile__card__wrapper--friends" style="width:100%;">
						<section class="profile__card__aligner">
							<div class="profile__card">
								...
								<div class="item-list--grid">
									<ul>
										<li v-for="friend in user.friends.slice(0, 5)" :key="friend.id" class="item item--friend">
                      <router-link :to="{ name: 'Home.Profile', params: { username : friend.user.username } }" class="item__content">
												<div class="item__icon">
													<imager :look="friend.user.look" class="imager"/>
												</div>
												<div class="item__text">
								...
								<!-- Footer -->
								<div class="profile__card__footer">
									<p @click="toggleModal('friends')" class="profile-modal__link">See all</p>
								</div>
								<!-- Modal -->
								<modal v-if="isModal('friends')" @close="toggleModal('friends')" title="Friends">
									<div class="item-list--stacked">
										<ul>
											<li v-for="friend in user.friends" :key="friend.id" class="item item--friend">
												<router-link :to="{ name: 'Home.Profile', params: { username : friend.user.username } }" class="item__content">
													<div class="item__icon">
														<imager :look="friend.user.look" class="imager"/>
													</div>
			...
			<div class="row">
				<footer class="wrapper wrapper--content">
					...
					<div class="profile__hearts"><i class="icon icon--heart"></i><i class="icon icon--heart"></i><i class="icon icon--heart"></i></div>
				</footer>
			</div>
		</div>
	</div>
