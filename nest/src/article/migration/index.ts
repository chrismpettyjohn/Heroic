...
export class Initial1567216000284 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query('CREATE TABLE `heroic_articles`  (\n' +
      ...
      '  CONSTRAINT `heroic_articles.user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE\n' +
      ');',
    );
    // Create foreign keys
    await queryRunner.query('ALTER TABLE `heroic_articles` \n' +
      // tslint:disable-next-line:max-line-length
      'ADD CONSTRAINT `heroic_articles.category_id` FOREIGN KEY (`category_id`) REFERENCES `heroic_articles_categories` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;',
    );
  }
