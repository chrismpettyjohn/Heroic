...
class Interactor {
  ...
  static async top() {
    try {
      let users = {
        credits: await _user2.default.query().whereIn('rank', [1, 2]).orderBy('credits', 'DESC').limit(3),
        points: await _user2.default.query().whereIn('rank', [1, 2]).orderBy('points', 'DESC').limit(3),
        online: await _user2.default.query().whereIn('rank', [1, 2]).joinEager('info').orderBy('info.online_time', 'DESC').limit(3)
      };
      return Promise.resolve(users);
    } catch (e) {
      return Promise.reject(e);
